...
- **Curtain effect centering**: Viewport auto-centers on the maqam's octave (or C3 if no maqam) using `centerMaqamOctave(tonicMidi, visibleCount)`. Formula: `padding = Math.floor((visibleCount - 11) / 2)`. The `Math.floor` ensures integer padding — at minimum width (12 sliders), padding=0 so tonic is flush left with no partial sliders. Centering is consistent across: plugin load, tuning system select, maqam select, preset load, and range slider double-click

...
export default function CustomSelect({ options, value, onChange, className = '', placeholder, searchable, selectedSuffix, disabled }: Props) {
  ...
  return (
    <div className={`custom-select ${className} ${open ? 'open' : ''} ${disabled ? 'disabled' : ''}`} ref={ref} onKeyDown={handleKeyDown}>
      <button className="cs-trigger" onClick={() => !disabled && setOpen(!open)} disabled={disabled}>

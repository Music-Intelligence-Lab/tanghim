...
- [2026-02-22](2026-02-22.md) — Sessions 25–27: Fixed octave centering formula (13 sliders). Context-aware enharmonic IPN labels via maqam detail API — Saba shows "Gb" while Hijaz shows "F#" for the same pitch. New `MaqamDetailResult` struct, maqam detail disk cache, async fetch in `applyMaqam()`, `degreeIpnMap` in tuning state JSON. Session 27: Native status bar — Clear Cache button, "Preset MIDI Map Config:" label, fixed MIDI device persistence bug.

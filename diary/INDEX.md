...
- [2026-02-22](2026-02-22.md) — Sessions 25–29: Fixed octave centering formula (13 sliders). Context-aware enharmonic IPN labels via maqam detail API — Saba shows "Gb" while Hijaz shows "F#" for the same pitch. New `MaqamDetailResult` struct, maqam detail disk cache, async fetch in `applyMaqam()`, `degreeIpnMap` in tuning state JSON. Session 27: Native status bar — Clear Cache button, "Preset MIDI Map Config:" label, fixed MIDI device persistence bug. Session 28: Reference Frequency Control — global concert pitch offset (±700 cents) with SVG arc knob, Hz input, semitone ±100 cent buttons, APVTS automation, state persistence. Resets to 0 on starting note change. Session 29: Fixed MIDI device persistence (dontSendNotification on ComboBox init), added realtime device list polling (2Hz), device open retry for late-initializing USB devices.

...
- [2026-02-19](2026-02-19.md) — Sessions 14–15: Remove output mode from Transmitter (MTS-ESP only), replace UI selector with read-only status display, M4L crash fix (plug_vst3), .amxd binary format, mode mapping fix, GitHub repo "tanghim" setup, plugin rename to "Tanghim" (ASCII constraint), live MTS-ESP status polling (2Hz), file-based receiver registry for type distinction (MTS-ESP/MPE/Mono PB badges)
